...
var allPathsSourceTarget = function (graph) {
  ...
  const res = []
  // Since I’m using BFS, I need a queue.
  // I know I want to start from node 0. Let me think about what exactly I need to store in the queue.
  const queue = []
  // If I only store the current node in the queue, I can traverse the graph, but when I reach the target, I won’t know the full path.
  ...
  // Now I’ll process the queue in standard BFS fashion.
  while (queue.length) {
    const current = queue.shift()
    // For each state, I extract the current node and the current path.
    const node = current.node, path = current.path

    // 9. 中止条件 If the current node is the target, then the current path is one complete answer.
    if (node === graph.length - 1) {
      res.push(path)
      continue
    }

    // 10. 如果不是终点，就扩展邻居 Otherwise, I expand all neighbors of the current node.
    for (const next of graph[node]) {

      // 11. When I go to a neighbor, I need to create a new state.The new node is next, and the new path should be the old path plus this next node.
      // 12. I create a new path with [...path, next] so each state has its own independent path.
      queue.push({
        node: next,
        path: [...path, next]
      })
