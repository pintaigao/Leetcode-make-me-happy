...
import { useFeedActions, useFeedIsStale, useFeedPosts, useFeedState } from '../store/feedStore';

const SCROLL_KEY = 'newsfeed:scrollY';

export default function FeedList() {
  const posts = useFeedPosts();
  const isStale = useFeedIsStale();
  const { hasMore, loading, loadingMore, error } = useFeedState();
  const { loadInitial, loadMore, refresh } = useFeedActions();
    ...
    <>
      {isStale ? (
        <div className="banner" role="status">
          This feed might be stale. <button className="btn" onClick={() => refresh()}>Refresh</button>
        </div>
      ) : null}
